...
const useCartUpdater = () => {
  ...
  const fetchTotal = async () => {
    ...
    try {
      ...
      setCartTotal({
        value:
          "There has been an error calculating the total. Please reload the page.",
      });
    }
    flagCartUpdate(false);
  };
  ...
  const optimisticallyUpdateTotal = () => {
    // NOTE: this is failing on transition when add to cart followed by cart quickly, fix applied for launch, solve properly on refactor
