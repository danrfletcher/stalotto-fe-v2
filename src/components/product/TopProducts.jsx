...
const TopProducts = () => {
    ...
    useEffect(() => {
        if (initialLoad && Array.isArray(filteredCompetitions)) {
            if (filteredCompetitions.length > 0) {
                setInitialCompetitionData(
                    structuredClone(filteredCompetitions),
                );
    ...
    const handleProducts = (category, i) => {
        //latest displays 25 most recent competitions, fetched on page load
        if (category === 'Latest') {
            setFilteredCompetitions(initialCompetitionData);
            handleActive(i);
            return;
        } else if (category === 'Now Live') {
            //now live fetches 25 most recent live competitions
            setFilteredCompetitions('This section is coming soon! 1');
            handleActive(i);
            return;
        } else if (category === 'Winners') {
            //winners fetches 25 most recently won competitions
            setFilteredCompetitions('This section is coming soon! 2');
            handleActive(i);
            return;
        } else if (category === 'Pending Draw') {
            //pending draw fetches 25 most recent competitions with no winningTicketID yet
                ...
                {filteredCompetitions === null ? (
                    <div className="card products_card browse_card loading_card">
                        <div className="tp_loader_container">
                            <PulseLoader
                                color="#a9afc3"
                                className="tp_pulse_loader"
                            />
                        </div>
                    </div>
                ) : typeof filteredCompetitions === 'string' ? (
                    <div className="card products_card browse_card">
                        <p className="comp_placeholder">
